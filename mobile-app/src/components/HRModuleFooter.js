...
/**
 * HRModuleFooter - Bullet Chamber Circular Spin Wheel Navigation
 */
const HRModuleFooter = ({ navigation, currentScreen }) => {
  ...
  const menuItems = ALL_HR_ITEMS.filter((item) => item.key !== currentScreen);
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (Math.abs(gestureState.dx) < 6 && Math.abs(gestureState.dy) < 6) {
          // It was a tap on overlay (or handled by bubble buttons)
          return;
        }
        startInertia();
      },
    ...
    <>
      ...
      <View style={styles.footer}>
        ...
        <View
          ...
          style={styles.hrButtonWrapper}
        >
          <TouchableOpacity
            activeOpacity={0.85}
            onPress={menuOpen ? () => closeMenu() : openMenu}
            style={[styles.hrButton, menuOpen && styles.hrButtonActive]}
          >
            {menuOpen ? (
          ...
          <Text style={[styles.footerLabel, menuOpen && { color: '#374151' }]}>HR</Text>
        </View>
