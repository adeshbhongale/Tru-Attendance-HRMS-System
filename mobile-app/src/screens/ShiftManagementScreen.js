...
const ShiftManagementScreen = ({ navigation }) => {
  ...
  const getStatusStyle = (status) => {
    ...
    if (s.includes('late')) return { bg: 'bg-amber-100', text: 'text-amber-700', border: 'border-amber-200' };
    if (s.includes('half')) return { bg: 'bg-orange-100', text: 'text-orange-700', border: 'border-orange-200' };
    if (s.includes('absent')) return { bg: 'bg-rose-100', text: 'text-rose-700', border: 'border-rose-200' };
    if (s.includes('present')) return { bg: 'bg-emerald-100', text: 'text-emerald-700', border: 'border-emerald-200' };
    if (s.includes('neutral')) return { bg: 'bg-indigo-100', text: 'text-indigo-700', border: 'border-indigo-200' };
        ...
        <Modal visible={showStatusModal} transparent animationType="fade">
          ...
          >
            <View className="bg-white rounded-t-[32px] p-6 pb-12">
              ...
              {['All', 'Present', 'Present-Late', 'Half-Day', 'Absent', 'Neutral'].map(s => (
                <TouchableOpacity
                  key={s}
                  onPress={() => {
                    setStatusFilter(s);
